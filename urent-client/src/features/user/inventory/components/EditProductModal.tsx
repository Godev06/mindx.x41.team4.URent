...
import { X, Plus, Trash2, Sparkles, Loader2, Zap } from "lucide-react";
    ...
    if (product && isOpen) {
      setNewProduct({
        ...
        statusQuantities: product.statusQuantities
          ...
          : { available: 1, rented: 0, overdue: 0 },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in duration-400">
      ...
      <div className="relative w-full max-w-5xl transform overflow-hidden rounded-[2.5rem] bg-white shadow-3xl transition-all dark:bg-slate-900 animate-in zoom-in-95 duration-300 ring-1 ring-black/5 dark:ring-white/5 border border-slate-100 dark:border-slate-800">
        ...
        >
          {/* Media Panel */}
          <div className="w-full md:w-5/12 bg-linear-to-b from-slate-50 to-slate-100 dark:from-slate-800/50 dark:to-slate-900/50 p-10 flex flex-col justify-center border-r border-slate-100 dark:border-slate-800 relative overflow-hidden">
          ...
          <div className="flex-1 p-10 md:p-14">
            <header className="mb-10 flex items-start justify-between">
              <div>
                ...
                <p className="mt-2 text-xs font-bold text-slate-400 uppercase tracking-widest">
                  {lang === "vi"
                    ? "Cập nhật chi tiết mặt hàng trong kho của bạn"
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-2">
                ...
                <input
                  className="w-full rounded-2xl border border-slate-100 bg-slate-50/50 dark:bg-slate-900/50 dark:border-slate-800 px-5 py-4 text-base font-bold outline-hidden focus:border-teal-500 focus:ring-4 focus:ring-teal-500/5 dark:text-white transition-all"
              ...
              <div className="grid grid-cols-3 gap-6">
                <div className="col-span-2 space-y-2">
                  ...
                  <select
                    className="w-full rounded-2xl border border-slate-100 bg-slate-50/50 dark:bg-slate-900/50 dark:border-slate-800 px-5 py-4 text-sm font-bold outline-hidden focus:border-teal-500 focus:ring-4 focus:ring-teal-500/5 dark:text-white transition-all appearance-none cursor-pointer"
              ...
              <div className="grid grid-cols-3 gap-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">
                    {t.inventoryAvailable || "Rảnh"}
                  </label>
                  <input
                    type="number"
                    min={0}
                    className="w-full rounded-2xl border border-slate-100 bg-slate-50/50 dark:bg-slate-900/50 dark:border-slate-800 px-5 py-4 text-sm font-bold outline-hidden focus:border-teal-500 focus:ring-4 focus:ring-teal-500/5 dark:text-white transition-all"
                    value={newProduct.statusQuantities.available}
                    onChange={(e) =>
                      setNewProduct((p) => ({
                        ...p,
                        statusQuantities: {
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">
                    {t.inventoryRented || "Thuê"}
                  </label>
                  <input
                    type="number"
                    min={0}
                    className="w-full rounded-2xl border border-slate-100 bg-slate-50/50 dark:bg-slate-900/50 dark:border-slate-800 px-5 py-4 text-sm font-bold outline-hidden focus:border-teal-500 focus:ring-4 focus:ring-teal-500/5 dark:text-white transition-all"
                    value={newProduct.statusQuantities.rented}
                    onChange={(e) =>
                      setNewProduct((p) => ({
                        ...p,
                        statusQuantities: {
                          ...p.statusQuantities,
                          rented: Number(e.target.value),
                        },
                      }))
                    }
                  />
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">
                    {t.inventoryOverdue || "Trễ"}
                  </label>
                  <input
                    type="number"
                    min={0}
                    className="w-full rounded-2xl border border-slate-100 bg-slate-50/50 dark:bg-slate-900/50 dark:border-slate-800 px-5 py-4 text-sm font-bold outline-hidden focus:border-teal-500 focus:ring-4 focus:ring-teal-500/5 dark:text-white transition-all"
                    value={newProduct.statusQuantities.overdue}
                    onChange={(e) =>
                      setNewProduct((p) => ({
                        ...p,
                        statusQuantities: {
                          ...p.statusQuantities,
                          overdue: Number(e.target.value),
                        },
                      }))
                    }
                  />
              ...
              <div className="space-y-2">
                ...
                <textarea
                  className="w-full rounded-2xl border border-slate-100 bg-slate-50/50 dark:bg-slate-900/50 dark:border-slate-800 px-5 py-4 text-sm font-bold outline-hidden focus:border-teal-500 focus:ring-4 focus:ring-teal-500/5 dark:text-white transition-all resize-none"
                  rows={3}
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input
                    className="w-full rounded-2xl border border-slate-100 bg-slate-50/50 dark:bg-slate-900/50 dark:border-slate-800 px-5 py-4 text-lg font-black outline-hidden focus:border-teal-500 focus:ring-4 focus:ring-teal-500/5 dark:text-white transition-all tabular-nums"
                    placeholder="0"
