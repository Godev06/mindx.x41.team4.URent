...
import { EditProductModal } from "../components/EditProductModal"; // <-- THÊM IMPORT
...
export default function InventoryPage() {
  ...
  const [editingItem, setEditingItem] = useState<InventoryItem | null>(null); // <-- THÊM STATE CHỈNH SỬA
    ...
    <div className="min-h-screen bg-transparent p-4 sm:p-6 md:p-8 font-sans selection:bg-teal-100 selection:text-teal-900">
          ...
          {filteredItems.length > 0 ? (
            filteredItems.map((item) => (
              <InventoryRow
                ...
                onEdit={(selectedItem) => setEditingItem(selectedItem)} // <-- MỞ MODAL SỬA
              />
            ))
          ) : (
      ...
      <EditProductModal 
        ...
        onSuccess={() => {
          ...
          fetchInventory();     // Tải lại dữ liệu mới nhất
        }} 
      />
    </div>
  );
