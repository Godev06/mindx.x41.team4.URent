          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`group flex items-center gap-3.5 px-4 py-3.5 rounded-2xl text-sm font-semibold transition-all duration-300 border ${active
                  ? "bg-gradient-to-r from-teal-500/15 to-cyan-500/5 text-cyan-300 border-cyan-500/30 shadow-md shadow-cyan-500/5"
                  : "text-slate-400 border-transparent hover:text-white hover:bg-white/[0.03]"
                  }`}
              >
                <Icon className={`h-5 w-5 transition-transform duration-300 group-hover:scale-110 ${active ? "text-cyan-400" : "text-slate-400 group-hover:text-white"}`} />
                {item.label}
              </button>
            );
          })}
