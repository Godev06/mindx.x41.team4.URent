  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-18 z-40 flex flex-col items-center border-r border-slate-200 bg-white py-5 shadow-[4px_0_24px_-10px_rgba(15,118,110,0.20)]">
      ...
      <div className="relative mt-6" ref={profileRef}>
        ...
        {isProfileOpen && (
          <div className="absolute bottom-12 left-0 w-48 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lg ring-1 ring-slate-900/5 z-50">
            {profileMenuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center gap-3 border-b border-slate-100 px-4 py-3 text-sm transition last:border-b-0 ${
                    item.isDanger
                ...
                >
                  <Icon
                    size={16}
                    strokeWidth={2}
                    className={item.isDanger ? "text-red-600" : "text-teal-600"}
                  />
