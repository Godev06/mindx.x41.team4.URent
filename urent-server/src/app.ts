...
app.get(['/api', '/api/v1'], (req, res) => {
    res.json({
        success: true,
        status: "running",
        message: "URent API v1 đang hoạt động ổn định. Vui lòng sử dụng các endpoint cụ thể hoặc xem tài liệu.",
        documentation: "/api-docs"
    });
});
...
app.use((req, res, _next) => {
    ...
    res.status(404).json({
        ...
        error: {
            ...
            suggestion: "Vui lòng kiểm tra lại chính tả hoặc tham khảo tài liệu hướng dẫn tại /api-docs"
        },
        timestamp: new Date().toISOString()
    });
});
